...
void main(){
    ...
    // Dim
    float dim = 1.0 - distance(vUv, vec2(0.0, 1.0)) * 1.75 ;

    vec4 newColor = vec4(dim);
    newColor = color + rimColor + (glow * 0.5 * color) * dim;
    newColor.a = dim * uAlpha * (bars + rim + glow) * flicker;;

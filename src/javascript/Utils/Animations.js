...
export default class Animations {
  ...
  setLoadingPage() {
    ...
    this.startWarnings = async () => {
      ...
      setTimeout(() => this.finishWarnings(), 1500 + 0.25 * 7 * 1000);
    };
    ...
    this.exitLoadPage = async () => {
      ...
      if (this.config.touch || this.config.isMobile) {
        [this.DOM.mouseMove, ...this.DOM.scrollDown].forEach((el) =>
    ...
    this.DOM.exitLoadBtn.forEach((btn) =>
      btn.addEventListener("click", (event) => this.exitBtnClick())
    );
  ...
  setEventListeners() {
    ...
    this.idxController.isInConstraints = (dir) => {
      if (["ArrowUp", "w", "i"].includes(dir)) {
        dir = -1;
      }
      if (["ArrowDown", "s", "k"].includes(dir)) {
        dir = 1;
      }

      if (
        (dir < 0 && self.IDX <= 0) ||
        (dir > 0 && self.IDX >= self.MAX_INDEX)
      ) {
        return false;
      }
      return true;
    ...
    this.idxController.ArrowUp = () => {
      this.IDX--;
      this.animationsProps[this.IDX].rotation = -Math.abs(
        this.animationsProps[this.IDX].rotation
      );
      this.direction = "up";
    };
    ...
    async function _wheel(event) {
      ...
      if (
        !self.idxController.isInConstraints(
      ...
      }, self.animationsProps[self.IDX].duration * 1000); // Seconds for Timeout
    }
  ...
  animateLights() {
    this.time.on("tick", () => {
      if (!this.world.lights.items.spotLight.target.intersected) {
        this.world.lights.items.spotLight.target.position.x =
          Math.cos(this.time.elapsed * 0.00105) * 2.5;
        this.world.lights.items.spotLight.target.position.y =
          Math.sin(this.time.elapsed * 0.00035) * 2.5;
      }
    });
  }
  ...
  animateCamera() {
    ...
    this.time.on("tick", () => {
      (this.camera.instance.position.x =
        Math.sin(this.camera.rotationAngle + this.mouse.x) *
        this.camera.distance),
        (this.camera.instance.position.z =
          Math.cos(this.camera.rotationAngle + this.mouse.x) *
          this.camera.distance),
  ...
  animateScrollDownDiv() {
    if (this.config.touch || this.config.isMobile) {
      gsap.to(".touched-scroll", {
        ...
        onComplete: () => {
          gsap.fromTo(
            [".touched-scroll", ".inner-touch-scroll"],
            {
              bottom: 0,
            },
            {
  ...
  async radioWavesAnimation() {
    ...
    this.world.radioWaves.forEach((wave, i, array) => {
      (function (w, idx, arr) {
        setTimeout(
          () => animate(w),
          0 + (duration / (arr.length - 1)) * (idx + 1) * 1000
        );
      })(wave, i, array);
    });
  }
  ...
  setScrollAnimations() {
    ...
    this.scrollAnimations.projectsComplete = () => {
      gsap.to(this.world.astronautContainer.position, {
        ...
        y: -41.278,
        z: -7.139,
        duration: this.animationsProps[6].duration * 0.1,
        ease: this.ease.power4Out,
      });
      gsap.to(this.world.astronautContainer.scale, {
        x: this.isDesktop() ? 1 : 0.5,
        y: this.isDesktop() ? 1 : 0.5,
        z: this.isDesktop() ? 1 : 0.5,
        duration: this.animationsProps[6].duration * 0.1,
        ease: this.ease.power2Out,
      });
    };
